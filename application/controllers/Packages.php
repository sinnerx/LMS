...
class Packages extends CI_Controller {
  ...
  $this->load->model('packagesmdata');
  $this->load->model('packagesm_data');
  ...
  $this->load->view('packagem/landing');
...
public function ajax_list()
  {
    $list = $this->packagesmdata->get_datatables();
    $data = array();
    $no = $_POST['start'];
      ...
      //add html for action
       $row[] = '<a href="../courses/index/'."".$package->packageid."".'"><i class="i i-checked"></i>enter</a> &nbsp;';
       ...
       $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->packagesmdata->count_all(),
            "recordsFiltered" => $this->packagesmdata->count_filtered(),
            "data" => $data,
        );
  ...
  {
     $data = $this->packagesmdata->get_by_id($packageid);
  ...
  //     );
  //   $insert = $this->packagesmdata->save($data);
  ...
  public function ajax_update()
  {
    ...
    $this->packagesmdata->update(array('packageid' => $this->input->post('packageid')), $data);
    echo json_encode(array("status" => TRUE));
  }

  public function ajax_delete($packageid)
  {
    $this->packagesmdata->delete_by_id($packageid);
    echo json_encode(array("status" => TRUE));
  }
  ...
  $userLevel = $this->nativesession->get( 'userLevel' );
    ...
    $this->load->helper('url');
       ...
       $this->load->view('templates/left_side_manager', $data);
       $this->load->view('templates/content_header', $data);
       $this->load->view('packagem/insert',$data);
       $this->load->view('templates/footer');
  ...
  $userLevel = $this->nativesession->get( 'userLevel' );
    ...
    if ($packageid !== null){
      ...
      $data['question'] = $this->packagesmdata->packageid($packageid);
      $data['page_title'] = 'Monte Carlo';
      $data['nav_title'] = 'package';
     ...
     /* if($this->session->userdata('logged_in'))
      {
       ...
       $this->load->view('templates/left_side_manager', $data);
       $this->load->view('templates/content_header', $data);
       $this->load->view('packagem/view', $data);
       $this->load->view('templates/footer');
     ...
     /* else
      ...
      $data['package'] = $this->packagesmdata->package();

      $this->load->view('packagem/index', $data);
...
function packagesm_data() 
  ...
  $userLevel = $this->nativesession->get( 'userLevel' );
    ...
    $this->packagesm_data->packages($data);
